...
def mine_player_salary(first_name: str, last_name: str, player_code = None):
    """
    ...
    player_code: str, default None
        the player code from sportsipy
        ...
        try:
            ...
            for mod in ['odd', 'even']:
                for row in table_body.find_all('tr', class_ = mod):
                    lst.append((
                        row.find('td', class_ = 'left').string, #Season
                        last_name,
                        first_name, 
                        player_code,
                        row.find_all('td', class_ = 'center')[2].string, #Average Annual Value
                        contract_year
                               ))
                    contract_year = 0
        except IndexError:
    ...
    df = pd.DataFrame(lst, columns=['season', 'last_name', 'first_name', 'player_code', 'AAV', 'contract_year'])
...
def build_player_salary_df():
    """
    Compiles a dataframe of yearly salaries of all NHL players from in the csv generated by the build_player_stats_df function.
    ...
    pathway = os.path.join(dirname, '../../data/')
    file_names = [
        'nhl_player_yearly_salaries',
        'error_players_salaries'
        ]
    file_num = ['', '']
    ...
    df_player_stats = pd.read_csv(pathway + 'full_nhl_player_yearly_stats.csv')
    n_players = len(df_player_stats.player_code.unique())
    df_player_stats = df_player_stats[['first_name', 'last_name', 'player_code']]
    df_player_stats = df_player_stats.drop_duplicates().reset_index(drop = True)
    
    assert len(df_player_stats) == n_players, 'Error with paring df_player_stats'
    
    first_name_list = df_player_stats.first_name.to_list()
    last_name_list = df_player_stats.last_name.to_list()
    player_code_list = df_player_stats.player_code.to_list()
    
    for i in range(n_players):
        time.sleep(2)
        try:
            df = pd.concat([df, mine_player_salary(first_name_list[i], last_name_list[i], player_code_list[i])])
        except:
            error_players.append((last_name_list[i], first_name_list[i], player_code_list[i]))
 
    df = df.reset_index(drop = True)

    df.to_csv(pathway + file_names[0] + str(file_num[0]) + '.csv')
    if error_players:
        pd.DataFrame(error_players).to_csv(pathway + file_names[1] + str(file_num[1]) + '.csv')
